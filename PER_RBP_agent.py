...
class PER_RBP_Agent():
    ...
    def learn(self, experiences, gamma):
        ...
        Params
        ======
            experiences (Tuple[torch.Tensor]): tuple of (s, a, r, s', done, tde, w) tuples
        ...
        self.memory.update_td_errs(TD_errors)
...
class ReplayBuffer:
    ...
    def add(self, state, action, reward, next_state, done):
        ...
        if not self.memory:
            ...
            max_td_error = max([e.td_error for e in self.memory])
            e = self.experience(state, action, reward,
                                next_state, done, max_td_error)

        self.memory.append(e)

    def update_td_errs(self, td_errors):
        """Update transition priorites in memory."""
        for sample, td_error in zip(self.tde_ptr[self.samples], td_errors):
    ...
    def alt_sample(self, beta, alpha):
        ...
        td_errs = np.array(
            [e.td_error for e in self.memory if e is not None])
        self.tde_ptr = np.argsort(td_errs)
        # note that you can reverse self.tde_ptr here if needed
        #self.tde_ptr = np.flip(self.tde_ptr)
        # add 1 to array in order to avoid divide by zero
        priorities = 1/(np.arange(len(td_errs))+1)
        ...
        weights = torch.from_numpy(
            np.vstack(((probabilities[self.tde_ptr[self.samples]] * N) ** -beta)/max_weight)).float().to(device)  # importance-sampling (IS) weights

        states = torch.from_numpy(
            np.vstack([self.memory[e].state for e in self.tde_ptr[self.samples]])).float().to(device)
        actions = torch.from_numpy(
            np.vstack([self.memory[e].action for e in self.tde_ptr[self.samples]])).long().to(device)
        rewards = torch.from_numpy(
            np.vstack([self.memory[e].reward for e in self.tde_ptr[self.samples]])).float().to(device)
        next_states = torch.from_numpy(
            np.vstack([self.memory[e].next_state for e in self.tde_ptr[self.samples]])).float().to(device)
        dones = torch.from_numpy(
            np.vstack([self.memory[e].done for e in self.tde_ptr[self.samples]]).astype(np.uint8)).float().to(device)
        td_errs = torch.from_numpy(
            np.vstack([self.memory[e].td_error for e in self.tde_ptr[self.samples]])).float().to(device)

        return (states, actions, rewards, next_states, dones, td_errs, weights)
